...
import { hashComparator } from '../../common/utils';
...
export default class Engine {
  ...
  _outputs = {};
  _inputs = {};
  _devices: { [id: string]: AbstractDevice } = {};
  _scripts = {};
  _timelines = {};
  _boards = {};
  _medias = {};
  ...
  _updateOutputs(data: OutputsState) {
  ...
  _updateInputs(data: InputsState) {
  ...
  _updateDevices(data: DevicesState) {
    this._devices = hashComparator(
      data,
      this._devices,
      (item) => {
        if (item.type === IOType.DMX) {
          return new DmxDevice(this._providers, item);
        } else if (item.type === IOType.ILDA) {
          return new IldaDevice(this._providers, item);
        } else if (item.type === IOType.MIDI) {
  ...
  _updateScripts(data: ScriptsState) {
  ...
  _updateTimelines(data: TimelinesState) {
  ...
  _updateBoards(data: BoardsState) {
  ...
  _updateMedias(data: MediasState) {
  ...
  _getDevice(deviceId) {
    ...
    if (device.type === IOType.DMX) {
      return new DmxDeviceProxy(device);
    } else if (device.type === IOType.ILDA) {
      return new IldaDeviceProxy(device);
    } else if (device.type === IOType.MIDI) {
  ...
  _runScript(id: ScriptId | null): void {
  ...
  _runTimeline(id: TimelineId | null): void {
  ...
  _runBoard(id: BoardId | null): void {
  ...
  _tickHandler(delta: number, initial: boolean = false) {
    if (this._currentScript) {
